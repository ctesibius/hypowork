...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" hypowork run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("hypowork onboard")} once, then retry ${pc.cyan("hypowork run")}.`);
      process.exit(1);
    }
  ...
  p.log.step("Starting Hypowork server...");
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  try {
    ...
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Hypowork server entrypoint.\n` +
          `Tried: ${devEntry}, @paperclipai/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `Hypowork server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`Hypowork server entrypoint did not export startServer(): ${label}`);
  }
