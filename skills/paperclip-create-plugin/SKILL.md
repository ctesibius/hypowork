...
description: >
  Create new Hypowork plugins with the current alpha SDK/runtime. Use when
...
# Create a Hypowork Plugin

Use this skill when the task is to create, scaffold, or document a Hypowork plugin.
...
For a plugin that lives outside the Hypowork repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.paperclip-sdk/`:
...
- is installed into Hypowork from an absolute local path during development
