...
description: >
  Create new agents in Hypowork with governance-aware hiring. Use when you need
  to inspect adapter configuration options, compare existing agent configs,
  draft a new agent prompt/config, and submit a hire request.
---

# Hypowork Create Agent Skill
...
2. Discover available adapter configuration docs for this Hypowork instance.
...
```sh
curl -sS "$PAPERCLIP_API_URL/api/workspaces/$PAPERCLIP_COMPANY_ID/agent-configurations" \
  -H "Authorization: Bearer $PAPERCLIP_API_KEY"
```
...
```sh
curl -sS -X POST "$PAPERCLIP_API_URL/api/workspaces/$PAPERCLIP_COMPANY_ID/agent-hires" \
  -H "Authorization: Bearer $PAPERCLIP_API_KEY" \
