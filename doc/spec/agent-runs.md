...
This spec defines how Hypowork actually runs agents while staying runtime-agnostic.
...
1. Hypowork is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Hypowork manages lifecycle, and on exit Hypowork parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `paperclip.skill` (shared Hypowork skill text block)
...
- Endpoint: `GET /api/workspaces/:workspaceId/events/ws`
...
8. `GET /api/workspaces/:workspaceId/events/ws`
   - websocket stream
