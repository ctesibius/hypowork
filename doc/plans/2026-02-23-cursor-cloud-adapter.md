...
This document defines the V1 design for a Hypowork adapter that integrates with
...
4. synthesized stdout events for Hypowork UI/CLI
...
3. **Agent callback auth to Hypowork** uses a bootstrap exchange flow (no long-lived Hypowork key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Hypowork endpoints, not full SKILL.md prompt injection.
...
## Hypowork Callback + Auth Flow (V1)
...
Goal: avoid putting long-lived Hypowork credentials in prompt text.

Flow:

1. Before launch/follow-up, Hypowork mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Hypowork validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Hypowork API calls.
...
1. Prompt includes a compact instruction to fetch skills from Hypowork.
...
4. Route by Cursor `agentId` to active Hypowork run context.
...
| Skill delivery | local skill dir injection | authenticated fetch from Hypowork skill endpoints |
| Hypowork auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Hypowork context.
3. Add richer PR metadata surfacing in Hypowork UI.
