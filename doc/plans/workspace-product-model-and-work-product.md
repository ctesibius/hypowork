...
Hypowork needs to support two very different but equally valid ways of working:
...
Today, Hypowork already has the beginnings of this model:
...
- making Hypowork feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Hypowork users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Hypowork deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Hypowork into a full code review product
...
This object must be recorded explicitly so that Hypowork can:
...
Hypowork should treat PRs as a type of work product linked back to:
...
Hypowork already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Hypowork must be a first-class deployment mode

Hypowork cannot assume that it is running on the same machine as the code.

In cloud deployments, Hypowork may:
...
- `Runtime service`: a process or service Hypowork owns or tracks for a workspace
...
It also lets cloud-hosted Hypowork deployments point at codebases and remotes without pretending the Hypowork host has direct filesystem access.
...
Without an explicit `execution workspace` record, Hypowork has nowhere stable to attach:
...
Hypowork needs a single place to show:
...
- Hypowork-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Hypowork owning the execution host
...
## 1. Hypowork host and execution host must be decoupled
...
without the Hypowork host itself running local git or local preview processes.
...
Hypowork should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Hypowork.
...
Hypowork should keep workspace automation centered on a small set of hooks:
...
Hypowork should provide the equivalent visibility in the project `Code` surface:
...
Hypowork should treat execution workspaces as the natural home for:
...
Hypowork should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Hypowork should continue treating cleanup policy as part of the core workspace model:
...
Hypowork should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Hypowork host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Hypowork should borrow Worktrunk's good execution patterns while keeping the broader Hypowork model:
...
That keeps Hypowork focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Hypowork deployments remain first-class

That is a better fit for Hypowork than either extreme:
