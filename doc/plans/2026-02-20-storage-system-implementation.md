...
Add a single storage subsystem for Hypowork that supports:
...
- API serves attachment bytes through authenticated Hypowork endpoints.
...
- `hypowork onboard` writes a valid `storage` config block by default.
- `hypowork configure --section storage` can switch between local and s3 modes.
...
- `hypowork doctor` reports actionable storage status.
