...
`PAP-447` asks how Hypowork should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Hypowork instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Hypowork:

- some operators use Hypowork to manage Hypowork and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Hypowork
- Claude and Codex expose different built-in affordances, so Hypowork should not overfit to one tool

## Core Product Decision

Hypowork should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Hypowork
...
- Hypowork should not assume that every adapter can see or use a host filesystem path directly
...
If Hypowork makes worktrees agent-first, it will blur:
...
- Hypowork core resolves and records execution workspace state
...
That gives Hypowork room to support:
...
- Hypowork handles workspace runtime services under the hood using project/workspace policy
...
Once Hypowork is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Hypowork should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Hypowork cannot infer
...
- when issue moves to done, should Hypowork auto-commit?
- should it auto-push?
...
Hypowork already has the right foundation for a project-first model.
...
For server-spawned local adapters, Hypowork already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Hypowork should mirror directly

Implication:

- for `codex_local`, Hypowork should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Hypowork should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Hypowork should separate:
...
Hypowork should be able to express intentions such as:
...
These are not specific to Hypowork. They are a common property of working in a dev workspace, whether local or remote.
...
Do not model this as a Hypowork-specific `devServer` flag.
...
Hypowork should distinguish between:
...
Hypowork should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Hypowork should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Hypowork
...
For runtime services specifically, Hypowork should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Hypowork-only dev-server flags
...
- same embedded DB / Hypowork instance can be reused through env/config injection when appropriate
...
- Hypowork can decide whether to reuse or start a fresh service deterministically
...
3. Test against Hypowork-on-Hypowork workflows first.
...
7. Workspace runtime services are modeled generically, not as Hypowork-specific dev-server toggles.
