...
Agents need API keys to authenticate with Hypowork. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Hypowork
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Hypowork generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Hypowork CLI
...
managed by the Hypowork server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Hypowork.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Hypowork SKILL.md (or a link to it)
   - What information Hypowork needs from the agent (e.g. webhook URL, adapter
...
5. Hypowork stores the pending registration.
...
7. On approval, Hypowork provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Hypowork issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "requiredFields": {
      ...
      "adapterType": "How Hypowork should send you heartbeats",
...
1. Generate a Hypowork invite link for the company.
...
4. A Hypowork company member approves the new agent.
5. Hypowork begins sending heartbeats to the OpenClaw webhook endpoint.
