...
3. In Hypowork UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in Hypowork UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from Hypowork invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in Hypowork with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in Hypowork UI that new issue exists.
