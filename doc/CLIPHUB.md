...
ClipHub is the public registry where people share, discover, and download Hypowork company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Hypowork what a package registry is to a programming language. Hypowork already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Hypowork — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Hypowork instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Hypowork spec. It contains:
...
```
Downloads the template and creates a new company in your local Hypowork instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Hypowork, export your company as a template, then publish:
...
ClipHub is a **separate service** from Hypowork itself. Hypowork is self-hosted; ClipHub is a hosted registry that Hypowork instances talk to.
...
| **Hypowork CLI** | `hypowork install`, `hypowork publish`, `hypowork cliphub sync` — built into Hypowork |
| **Hypowork UI** | "Browse ClipHub" panel in the Hypowork web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Hypowork) |
| Backend | TypeScript + Hono (consistent with Hypowork) |
| Database | PostgreSQL |
...
4. Run `hypowork install cliphub:acme/lean-saas-shop`
5. Hypowork creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Hypowork until it works well
2. Export: `hypowork export --template my-agency`
3. Publish: `hypowork publish cliphub my-agency`
...
3. Install just that agent: `hypowork install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Hypowork

ClipHub is **not required** to use Hypowork. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Hypowork what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `hypowork install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Hypowork web UI
- [ ] `hypowork cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Hypowork's own philosophy)
