...
Hypowork V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Hypowork-shaped design.

Assumption for this document: Hypowork is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Hypowork objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Hypowork.

The main conclusion is:

- Hypowork should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Hypowork should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Hypowork should use multiple extension classes instead of one generic plugin bag:
...
If Hypowork does this well, the examples you listed become straightforward:
...
Relevant Hypowork files reviewed for current extension seams:
...
It is too dangerous for Hypowork core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Hypowork — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Hypowork should absolutely do this.
...
Hypowork should copy this discipline.
...
Hypowork should adopt this style for plugin actions, automations, and UI schemas.
...
Hypowork can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Hypowork too.

## What Hypowork Should Not Copy Directly
...
Hypowork is a control plane for an operator-managed instance with company objects.
...
Hypowork is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Hypowork should not auto-load arbitrary code from a workspace repo like `.paperclip/plugins` or project directories.
...
For Hypowork, equivalent hooks into:
...
Hypowork should not allow plugins to silently replace:
...
For Hypowork it would be risky because it combines:
...
Hypowork should require an explicit operator install step.

## Why Hypowork Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Hypowork |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Hypowork instance |
...
That means Hypowork should borrow the good ideas from `opencode` but use a stricter architecture.

## Hypowork Already Has Useful Pre-Plugin Seams

Hypowork has several extension-like seams already:
...
This is good news.
Hypowork does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Hypowork Plugin Model
...
Hypowork already has this pattern implicitly:
...
- `plugin` means integration code that talks to Hypowork through a typed plugin protocol and capability model
...
- invoke explicit Hypowork actions through the API
...
Hypowork should treat plugin installation as a global instance-level action.

Examples:

- install `@paperclip/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Hypowork objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Hypowork already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Hypowork too.
...
A `@paperclipai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Hypowork instance.

A `create-paperclip-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Hypowork achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Hypowork server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Hypowork's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.
...
## A Concrete SDK Shape For Hypowork
...
export default definePlugin({
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Hypowork entities
    });
...
Any Hypowork plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Hypowork objects, those are plugin data, not enable/disable boundaries.
...
- reuse core tables when the data is clearly part of Hypowork itself
...
If a concept is becoming part of Hypowork's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Hypowork itself
...
### 3. Scope plugin data to Hypowork objects before adding custom schemas

A lot of plugin data naturally hangs off existing Hypowork objects:
...
If Hypowork eventually needs extension-owned tables, I would only allow that for:
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Hypowork.
- Board reviews the diff after a run without leaving Hypowork.
...
This plugin syncs Hypowork work with Linear. It is useful for:

- importing backlog from Linear
- linking Hypowork issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Hypowork and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Hypowork.
- Hypowork issue status changes push to Linear and Linear comments arrive back through webhooks.
...
This plugin syncs Hypowork issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Hypowork issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Hypowork task governance
...
- Plugin settings:
  ...
  - whether new Hypowork issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Hypowork issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Hypowork.
- GitHub webhooks update status/comment state in Hypowork.
- A PR is linked back to the Hypowork issue so the board can follow delivery status.
...
This plugin surfaces external metrics and dashboards inside Hypowork. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Hypowork issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Hypowork plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Hypowork dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Hypowork issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Hypowork core; keep it as additive signal and issue creation
...
This plugin pulls Stripe revenue and subscription data into Hypowork. It is useful for:
...
- Failed payment spikes or churn events can generate Hypowork issues for follow-up.
...
- Stripe data should stay additive to Hypowork core
...
Hypowork should not implement "an OpenCode-style generic in-process hook system."
Hypowork should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Hypowork
