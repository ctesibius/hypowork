# Hypowork Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Hypowork's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Hypowork app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Hypowork HTTP APIs directly.
...
Hypowork plugin design is based on the following assumptions:

1. Hypowork is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Hypowork business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Hypowork core.
...
2. Let plugins add major capabilities without editing Hypowork core.
...
The single Hypowork deployment an operator installs and controls.

### 5.2 Company

A first-class Hypowork business object inside the instance.
...
A trusted in-process extension loaded directly by Hypowork core.
...
An installable instance-wide extension package loaded through the Hypowork plugin runtime.
...
Hypowork has two extension classes.
...
Hypowork already has a concrete workspace model:
...
Plugins live under the Hypowork instance directory.
...
Hypowork should add CLI commands:

- `pnpm hypowork plugin list`
- `pnpm hypowork plugin install <package[@version]>`
- `pnpm hypowork plugin uninstall <plugin-id>`
- `pnpm hypowork plugin upgrade <plugin-id> [version]`
- `pnpm hypowork plugin doctor <plugin-id>`
...
Plugins may contribute tools that Hypowork agents can use during runs.
...
- Hypowork server starts one worker process per installed plugin
...
Receives one typed Hypowork domain event.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Hypowork secret provider system rather than a plain text input
...
1. Core Hypowork data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Hypowork objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Hypowork product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Hypowork later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Hypowork secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm hypowork plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Hypowork server. This is a normative requirement, not optional.
...
For developing a plugin against a running Hypowork instance:

- The operator installs the plugin from a local path: `pnpm hypowork plugin install ./path/to/plugin`
...
Hypowork should not implement a generic in-process hook bag modeled directly after local coding tools.

Hypowork should implement:
...
That is the complete target design for the Hypowork plugin system.
