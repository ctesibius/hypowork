...
/** Org chart API shape (matches client `OrgNode` / Express `toLeanOrgNode`). */
...
/** Manager link only if it points at another agent in the same in-memory set (else treat as root). */
function effectiveManagerInCompany(reportsTo: string | null, idSet: Set<string>): string | null {
  if (reportsTo == null || !idSet.has(reportsTo)) return null;
  return reportsTo;
}

/**
 * If every agent has a non-null `reportsTo` that stays inside the company graph, the naive tree has
 * no roots unless at least one agent has `reportsTo === null`. Cycles also yield no roots. Break each
 * cycle by forcing the lexicographically smallest id in the cycle to be a root for layout only.
 */
function forcedCycleRootIds(
  rows: Array<{ id: string; reportsTo: string | null }>,
  eff: (r: { id: string; reportsTo: string | null }) => string | null,
): Set<string> {
  ...
  for (const start of rows) {
    ...
    while (cur) {
      if (path.includes(cur)) {
        ...
        forced.add(cycle.reduce((a, b) => (a < b ? a : b)));
        break;
      }
      path.push(cur);
      const r = map.get(cur);
...
interface UpdateAgentOptions {
  ...
  /** Board-only: allow `terminated` → `idle` (restore). */
  restoreFromTerminated?: boolean;
}
...
export function agentService(db: Db) {
  ...
  async function updateAgent(
    ...
    if (existing.status === "terminated" && data.status && data.status !== "terminated") {
      const restoringToIdle =
        options?.restoreFromTerminated === true && data.status === "idle";
      if (!restoringToIdle) {
  ...
  return {
    ...
    orgForCompany: async (companyId: string) => {
      ...
      const forcedRoots = forcedCycleRootIds(
        normalizedRows.map((r) => ({ id: r.id, reportsTo: r.reportsTo })),
        (r) => effectiveManagerInCompany(r.reportsTo, idSet),
      );

      const byManager = new Map<string | null, typeof normalizedRows>();
      for (const row of normalizedRows) {
        const key = forcedRoots.has(row.id) ? null : eff(row);
        const group = byManager.get(key) ?? [];
        group.push(row);
        byManager.set(key, group);
      }

      const buildLean = (managerId: string | null): OrgTreeNode[] => {
        ...
        return members.map((member) => ({
          ...
          reports: buildLean(member.id),
        }));
      };

      const result = buildLean(null);
