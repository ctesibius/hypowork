...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Hypowork host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Hypowork is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm hypowork allowed-hostname ${apiHost}`
    });
  }
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Hypowork hostname is reachable and allowed via `pnpm hypowork allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Hypowork API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Hypowork OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Hypowork generates and persists one during join so pairing approvals are stable.
    ...
    The board approves the join request in Hypowork before key claim is allowed.
    ...
    ## Step 4: Install Hypowork skill in OpenClaw
    ...
    ${
      ...
      "Ensure Hypowork is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Hypowork base URLs to try");
    ...
    appendBlock(`
      ...
      - then restart Hypowork and retry onboarding.
    `);
  }
