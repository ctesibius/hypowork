...
import { mergeReportingEnvForHeartbeat } from "../services/reporting-env-merge.js";
...
export function agentRoutes(db: Db) {
  ...
  async function assertCanUpdateAgent(req: Request, targetAgent: { id: string; companyId: string }) {
    assertWorkspaceAccess(req, targetAgent.companyId);
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const body = req.body as { adapterConfig: Record<string, unknown>; agentId?: string };
      let inputAdapterConfig = { ...body.adapterConfig } as Record<string, unknown>;
      if (body.agentId) {
        const child = await db
          .select({ reportsTo: agentsTable.reportsTo })
          .from(agentsTable)
          .where(and(eq(agentsTable.id, body.agentId), eq(agentsTable.companyId, companyId)))
          .limit(1)
          .then((rows) => rows[0]);
        if (child) {
          inputAdapterConfig = await mergeReportingEnvForHeartbeat(
            db,
            companyId,
            { reportsTo: child.reportsTo, adapterConfig: body.adapterConfig },
            inputAdapterConfig,
          );
        }
      }
  ...
  router.get("/workspaces/:workspaceId/org", async (req, res) => {
    ...
    const withHumanFlag = (node: any): any => ({
      ...node,
      isHuman: false,
      reports: Array.isArray(node.reports) ? node.reports.map(withHumanFlag) : [],
    });
    res.json(Array.isArray(tree) ? tree.map(withHumanFlag) : []);
  });
  ...
  router.patch("/agents/:id/reports-to", async (req, res) => {
    ...
    const reportsTo =
      typeof req.body?.reportsTo === "string" && req.body.reportsTo.trim().length > 0
        ...
        : null;
    const updated = await svc.update(id, { reportsTo });
    if (!updated) {
      res.status(404).json({ error: "Agent not found" });
      return;
    }
