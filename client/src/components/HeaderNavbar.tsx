    ...
    <Popover>
      ...
      <PopoverContent align="end" className="w-[min(100vw-2rem,26rem)] max-h-[min(70vh,22rem)] overflow-y-auto">
        <p className="text-xs leading-relaxed text-muted-foreground">
          ...
          <kbd className="rounded border border-border px-1 py-0.5 text-[10px] text-foreground">Enter</kbd> after editing the
          title (double-click the tab) still triggers save when focus moves; edits here sync to the same document body.
        </p>
      </PopoverContent>
    </Popover>
  );
...
function DocumentDetailChromeCluster() {
  const { documentDetailChrome } = useBreadcrumbs();
  if (!documentDetailChrome) return null;
  return (
