...
export function CodeDrawingElement(
  ...
  const { removeNode, image, loading, error: renderError } = useCodeDrawingElement({
    element,
  });
  ...
  const selectionCollapsed = useScopedEditorSelector(
    ...
    >
      ...
      {showImage ? (
        ...
        >
          ...
          {!loading && renderError && (
            <div className="max-w-full px-2 text-center text-sm text-destructive">
              {renderError}
            </div>
          )}
          {!loading && !renderError && image && (
            <img
              src={image}
              alt="Code drawing"
