...
export const heartbeatsApi = {
  list: (companyId: string, agentId?: string, limit?: number) => {
    ...
    return api.get<HeartbeatRun[]>(`/workspaces/${companyId}/heartbeat-runs${qs ? `?${qs}` : ""}`);
  },
  ...
  liveRunsForCompany: (companyId: string, minCount?: number) =>
    api.get<LiveRunForIssue[]>(`/workspaces/${companyId}/live-runs${minCount ? `?minCount=${minCount}` : ""}`),
