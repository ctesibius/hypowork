...
Kitchen Sink is the first-party reference plugin that demonstrates nearly the full currently implemented Hypowork plugin surface in one package.
...
pnpm hypowork plugin install ./packages/plugins/examples/plugin-kitchen-sink-example
```

Or install it from the Hypowork plugin manager as a bundled example once this repo is built.
...
- Some SDK-defined host surfaces still depend on the Hypowork host wiring them visibly; this package aims to exercise the currently mounted ones and make the rest obvious.
