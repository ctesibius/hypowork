/**
 * Core types for the Hypowork plugin worker-side SDK.
 *
...
export interface ScopeKey {
  /** What kind of Hypowork object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Hypowork UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Hypowork domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Hypowork domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Hypowork secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Hypowork secret provider (e.g. `"MY_API_KEY"`).
   *
