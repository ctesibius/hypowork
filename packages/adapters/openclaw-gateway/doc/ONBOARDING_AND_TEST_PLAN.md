...
## Scope
This plan is now **gateway-only**. Hypowork supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Hypowork skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Hypowork task.
...
pnpm --filter hypowork typecheck
```
