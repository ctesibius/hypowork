...
Build a custom adapter to connect Hypowork to any agent runtime.
...
2. Build environment with `buildHypoworkEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `hypowork run --watch` using `picocolors`.
...
Make Hypowork skills discoverable to your agent runtime without writing to the agent's working directory:
