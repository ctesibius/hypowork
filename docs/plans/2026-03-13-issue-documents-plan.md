...
Add first-class **documents** to Hypowork as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Hypowork workflow/docs.
...
Update the Hypowork agent workflow so planning no longer edits the issue description.
