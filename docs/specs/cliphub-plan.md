# ClipHub: Marketplace for Hypowork Team Configurations

> The "app store" for whole-company AI teams — pre-built Hypowork configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Hypowork-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Hypowork company (agents, projects, governance) |
...
A complete Hypowork company configuration:
...
Single-agent configurations designed to plug into a Hypowork org:
...
Portable skill files that any Hypowork agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Hypowork's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum Hypowork version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Hypowork company
...
| `POST` | `/api/listings/:id/install` | Install to Hypowork company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Hypowork company (or create new)
  → ClipHub API calls Hypowork API to:
  ...
  → Redirect to Hypowork dashboard with new team running
```
...
### 5.3 Creator: Export from Hypowork → Publish

```
Running Hypowork company → "Export as Blueprint" (CLI or UI)
  → Hypowork exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Hypowork brand blue/purple)
...
| Install flow | One-click deploy to Hypowork company |
...
{
  "targetCompanyId": "uuid",         // Existing Hypowork company
...
3. For each agent in blueprint:
   - `POST /api/workspaces/:workspaceId/agents` (if `paperclip-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Hypowork
- **Backend**: Node.js API (or extend Hypowork server)
- **Database**: Postgres (can share Hypowork's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Hypowork auth (or OAuth2)

### 9.2 Integration with Hypowork

ClipHub can be:
- **Option A**: A separate app that calls Hypowork's API to install blueprints
- **Option B**: A built-in section of the Hypowork UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Hypowork UI and API.
...
- [ ] Install flow: blueprint → Hypowork company
...
- [ ] "Export from Hypowork" CLI command
