...
How to get OpenClaw running in a Docker container for local development and testing the Hypowork OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Hypowork includes an end-to-end join smoke harness:
...
- probes and prints a Hypowork host URL that is reachable from inside OpenClaw Docker
...
If your Hypowork deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Hypowork server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Hypowork URL (typically `http://host.docker.internal:3100`).
- If Hypowork rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm hypowork allowed-hostname host.docker.internal
```

Then restart Hypowork and rerun the smoke script.
