...
Hypowork supports two runtime modes with different security profiles.
...
```sh
pnpm hypowork allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Hypowork emits a one-time claim URL at startup:
...
```sh
PAPERCLIP_DEPLOYMENT_MODE=authenticated pnpm hypowork run
```
